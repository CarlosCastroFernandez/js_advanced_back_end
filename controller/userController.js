...
const login = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(200)
        .send("Este usuario no existe, credemciales erroneas");
    ...
    if (!isMatch)
      return res
        .status(200)
        .send({ status: "Failed", message: "Credenciales errones" });
    if (user.role === "user")
      return res
        .status(200)
        .send({ status: "Failed", message: "No tienes permisos para acceder" });
    const payload = {
      _id: user._id,
      email: user.email,
      role: user.role,
    ...
    return res
      .status(200)
      .json({ status: "Success", data: user, token, tokenRefresh });
  } catch (e) {
...
const editUser = async (req, res) => {
  ...
  try {
    ...
    const userEdit = await userModel
      .findByIdAndUpdate(identification, userNew, {
        new: true,
        runValidators: true,
      })
    ...
    if (!userEdit)
      return res
        .status(404)
        .send({ status: "Failed", message: "No hay usuario que editar" });
    return res.status(200).send({ status: "Success", data: userEdit });
  } catch (error) {
    return res
      .status(200)
      .send({ status: "Failed", message: "Hubo algun error " + error.message });
  }
};
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (!userBorrado)
      return res
        .status(200)
        .send({ status: "Failed", message: "No se ha encontrado al usuario" });
